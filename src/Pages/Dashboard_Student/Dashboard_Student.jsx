        ...
        <div className='max-w-screen-xl mx-2 lg:mx-auto mt-6 lg:mt-8'>
            ...
            <div className='grid lg:grid-cols-3 gap-4 lg:gap-6'>
                ...
                <div className='bg-white dark:bg-dark text-dark dark:text-white px-4 py-6 rounded-lg'>
                    ...
                    <h2 className='p-2 pb-0 text-4xl font-bold'>{ reviews?.length }</h2>
                </div>
            </div>
            {
                selectedContents === 'selectedClasses' &&
                <>
                    ...
                    <div className='grid grid-cols-2 lg:grid-cols-4 gap-2 lg:gap-4'>
                        {
                            classes.length > 0 &&
                            classes?.map( classData => <SelectedClassesCard key={ classData._id } classData={ classData } refetch={ refetch } /> )
                        }
                    </div>
                </>
            }
            {
                selectedContents === 'enrolledClasses' &&
                <>
                    ...
                    <div className='grid grid-cols-2 lg:grid-cols-3 gap-2 lg:gap-6 mb-8'>
                        {
                            enrolledClasses.length > 0 ?
                                enrolledClasses?.map( classData => <EnrolledClassesCard key={ classData._id } classData={ classData } /> )
                                :
